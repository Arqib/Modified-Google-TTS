...
for key in "${!SHORT[@]}"
  do
    ...
    youtube-dl "http://translate.google.com/translate_tts?ie=UTF-8&client=tw-ob&q=$NEXTURL&tl=En-gb" 
done

###rename "s/unknown_video/mp3/" *.unknown_video
#while read line; do google-TTS.sh "$line"; done < test.txt
